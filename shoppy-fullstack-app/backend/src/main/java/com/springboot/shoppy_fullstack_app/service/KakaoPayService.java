...
// ----------------------------------------------------
    public KakaoReadyResponse kakaoPayReady(KakaoPay kakaoPay) {
        ...
        // params에 설정하는 값은 전부 문자열로 변경할것.
        MultiValueMap<String, String> params = new LinkedMultiValueMap<>();
        params.add("cid", CID);
        params.add("partner_order_id", orderId);                    // ✅ 꼭 필요! 가맹점 ID
        params.add("partner_user_id", userId);                      // 유저ID
        params.add("item_name", kakaoPay.getItemName());            // 판매 상품 명
        ...
        params.add("approval_url", "http://localhost:8080/payment/qr/success?orderId=" + orderId); // 성공
        params.add("cancel_url",   "http://localhost:8080/payment/qr/cancel?orderId=" + orderId); // 취소
        params.add("fail_url",     "http://localhost:8080/payment/qr/fail?orderId=" + orderId); // 실패
    ...
    // ----------------------------------------------------
    // 최종 결제 승인 (Approve) 반드시 approve로 이름지어 만들것.
    // ----------------------------------------------------
